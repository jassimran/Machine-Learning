package abc.assignment4.util;

import abc.assignment4.BasicGridWorld;
	...
	public void runValueIteration(BasicGridWorld gen, Domain domain,
		...
		for(int numIterations = increment;numIterations<=MAX_ITERATIONS;numIterations+=increment ){
			...
			vi = new ValueIteration(
					...
					0.99,
					hashingFactory,
					.001, numIterations); //Added a very high delta number in order to guarantee that value iteration occurs the max number of iterations
...
//		new EpisodeSequenceVisualizer(v, domain, Arrays.asList(ea));
	...
	public void runQLearning(BasicGridWorld gen, final Domain domain,
		...
		LearningAgentFactory qLearningFactory = new LearningAgentFactory() {
			...
			public LearningAgent generateAgent() {
				return new QLearning(domain, 0.99, hashingFactory, 0.99, 0.99);
			}
		};
